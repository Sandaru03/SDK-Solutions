...
export default function Hero({ isDark }: HeroProps) {
  ...
  useEffect(() => {
    const intervals = stats.map((stat, index) => {
      const stepTime = 20
      const increment = Math.ceil(stat.value / (2000 / stepTime)) // animate in 2s
  ...
  return (
    <section id="home" className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center pt-20 px-4">
        <div className="max-w-4xl mx-auto text-center space-y-8">
          ...
          <div className="space-y-4 animate-fade-in-up">
            ...
            <p className="text-xl sm:text-2xl text-gray-200 font-light">{slides[currentSlide].subtitle}</p>
          </div>

          {/* Subtitle */}
          <p className="text-lg sm:text-xl text-gray-200 max-w-2xl mx-auto leading-relaxed animate-fade-in-up animation-delay-200">
      ...
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center hover:bg-white/20 transition-all duration-300 group z-20"
      >
        <ChevronLeft className="w-6 h-6 text-white group-hover:scale-110 transition-transform" />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center hover:bg-white/20 transition-all duration-300 group z-20"
      >
